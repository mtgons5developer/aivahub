...
fine_tuning_job = openai.FineTunes.create(train_file='gpt-4-0314.txt', base_model='gpt2')


class ChatOpenAI(LLMChain):
    def __init__(self, fine_tuned_model=None, temperature=0.8):
...
def openAI(fine_tuned_model=None):
    try:
        # Read the CSV file
        with open("csv/B00UFJNVTS - Heat Resistant Oven Mitts (2 pack)_ High Temperatu 2023-06-07.csv", "r") as file:
            ...
            chat_llm = ChatOpenAI(fine_tuned_model=fine_tuned_model, temperature=0.8)
            ...
            for i, row in enumerate(csv_reader, start=1):
                ...
                # Generate a prompt using the title and body
                prompt = f"{title}\n{body}\n"

                # Send the prompt to the language model for analysis
                response = completion_with_retry(prompt)

                # Process the response and perform actions based on the rating
                # (e.g., classify sentiment, extract keywords, etc.)
                # ...

                # Optional: Print the response from the language model
                print(f"Response #{i}: {response.choices[0].text.strip()}\n")

            # Save the fine-tuned model on the fine-tuning job
            if chat_llm.fine_tuning_job:
                chat_llm.fine_tuning_job.create_checkpoint()
...
openAI(fine_tuned_model='aivahub-3.5', fine_tuning_job=fine_tuning_job)
