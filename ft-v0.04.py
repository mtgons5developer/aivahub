...
def check_for_violations(review):
    ...
    # Generate text based on the review
    ...
    violations = []
    if "Does the following review comply with all 14 of Amazon's guidelines?" in generated_text:
        start_index = generated_text.index("Does the following review comply with all 14 of Amazon's guidelines?")
        end_index = generated_text.index("If not, which guideline(s) does it potentially violate?")
        guideline_text = generated_text[start_index:end_index]
        guideline_text = guideline_text.replace("\n", "")
        violations = [line.strip() for line in guideline_text.split("-") if line.strip()]
            ...
            with open("formatted_reviews.txt", "w") as output_file:
                ...
                for i, row in enumerate(csv_reader, start=1):
                    ...
                    # Check if the rating value is 4 or 5
                        ...
                        # # Extract guideline violations from the generated text
                        # violations = extract_violations(review)
                        # if violations:
                        ...
                        # Check for guideline violations
                        violations = check_for_violations(review)
